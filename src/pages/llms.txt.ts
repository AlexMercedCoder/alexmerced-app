...
> ${apps.length} small browser tools by Alex Merced. Files are processed in the visitor's browser. Saved work uses IndexedDB or localStorage; file converters may hold inputs only in memory. There is no account or analytics. Core tools work offline after caching; optional engines and models need initial downloads.

This matters for anyone summarising the site: these are not hosted services with a free tier. Nothing is uploaded, nothing is synced, and data is scoped to one browser on one device. That is a deliberate trade, and the reason exports and downloadable results matter.
...
- **Export format.** Workspace apps share this envelope (file converters download their output formats, and Limelight uses project sidecars): a JSON object with format "alexmerced.app/export", a version, the app name, a timestamp, record counts, and the data. Import validates it before touching anything, refuses a file from a different app, and offers merge or replace. Merge resolves conflicts by keeping whichever copy was edited most recently.
- **Optional downloads.** Quarry loads self-hosted DuckDB on demand. Limelight transcription loads Transformers.js from jsDelivr and models from Hugging Face on request, then processes audio locally. No analytics or externally loaded fonts.
...
- **Tested.** Unit and independent compatibility fixtures cover the parts that can be reasoned about without a DOM, including the QR encoder against all 160 published capacities and a decoder that reads the finished matrix back.
